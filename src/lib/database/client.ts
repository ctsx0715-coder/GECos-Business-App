...
// Next.js hot reload would otherwise open a new pool on every edit.
const globalForPrisma = globalThis as unknown as {
  nopediRawDb?: PrismaClient;
  nopediDb?: ExtendedClient;
};
...
export const db: ExtendedClient =
  globalForPrisma.nopediDb ?? buildExtendedClient(rawDb);

if (process.env.NODE_ENV !== "production") {
  globalForPrisma.nopediRawDb = rawDb;
  globalForPrisma.nopediDb = db;
}
