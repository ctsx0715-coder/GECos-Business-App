...
export function PreviewBanner({
  children = "Preview — designed, not built. Nothing on this screen is saved, and the figures are demonstration data.",
}: {
  children?: ReactNode;
}) {
  return (
    <div className="mb-6 flex items-start gap-3 rounded-xl border border-warning/30 bg-warning-soft px-4 py-3">
      <span className="mt-0.5 shrink-0 rounded-md bg-warning/15 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-widest text-warning">
        Preview
      </span>
      <p className="text-xs leading-relaxed text-warning">{children}</p>
    </div>
  );
}
