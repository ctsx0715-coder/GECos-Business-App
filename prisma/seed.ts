...
async function main() {
  ...
  await withSystemContext(nopedi.organisation.id, async () => {
    ...
    // ---- HR ----------------------------------------------------------------
    // The people, the leave policy and the tickets they hold all come from
    // prisma/hr-demo.ts, because the backfill script writes the same dataset
    // into a database that is already live. Two copies of Nopedi's staff would
    // drift apart within a week of the first correction.
    await seedHrDemo({
      organisationId: nopedi.organisation.id,
  ...
  const counts = {
    ...
    employees: await rawDb.employee.count(),
    certifications: await rawDb.complianceItem.count(),
    leaveRequests: await rawDb.leaveRequest.count(),
  };
